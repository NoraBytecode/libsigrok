...
/* Set the next event to wait for in uni_t_mso2_receive */
static void uni_t_mso2_set_wait_event(struct dev_context* devc, enum wait_events event) {
    if (event == WAIT_STOP)
        devc->wait_status = 2;
    else
        devc->wait_status = 1;
 ...
 */
static int uni_t_mso2_event_wait(const struct sr_dev_inst* sdi, char status1, char status2) {
    char *buf, c;
    ...
    if (devc->wait_status == 2) {
        ...
        uni_t_mso2_set_wait_event(devc, WAIT_NONE);
    }
 ...
 * Agilents have. The UNI-Ts don't seem to have anything like this.
 *
 ...
 */
static int uni_t_mso2_trigger_wait(const struct sr_dev_inst* sdi) {
    struct dev_context* devc;
    long s;
    ...
    if (devc->timebase < 0.0499) {
        ...
        return uni_t_mso2_event_wait(sdi, 'T', 'A');
    }
}

/* Wait for scope to got to "Stop" in single shot mode */
static int uni_t_mso2_stop_wait(const struct sr_dev_inst* sdi) {
    return uni_t_mso2_event_wait(sdi, 'S', 'S');
}

/* Check that a single shot acquisition actually succeeded on the DS2000 */
static int uni_t_mso2_check_stop(const struct sr_dev_inst* sdi) {
    struct dev_context* devc;
    ...
    if (ch->type == SR_CHANNEL_LOGIC) {
        if (uni_t_mso2_config_set(sdi, ":WAV:SOUR LA") != SR_OK)
            return SR_ERR;
    }
    else {
        if (uni_t_mso2_config_set(sdi, ":WAV:SOUR CHAN%d",
                                  ch->index + 1) != SR_OK)
            return SR_ERR;
    }
    /* Check that the number of samples will be accepted */
    if (uni_t_mso2_config_set(sdi, ":WAV:POIN %d",
    ...
    if (tmp & 0x10) {
        ...
        g_usleep(500 * 1000);
        uni_t_mso2_config_set(sdi, ":SING");
        uni_t_mso2_set_wait_event(devc, WAIT_STOP);
        return SR_ERR;
    }
...
static int uni_t_mso2_block_wait(const struct sr_dev_inst* sdi) {
    char *buf, c;
    struct dev_context* devc;
    time_t start;
    int len, ret;
...
SR_PRIV int uni_t_mso2_config_set(const struct sr_dev_inst* sdi, const char* format, ...) {
...
SR_PRIV int uni_t_mso2_capture_start(const struct sr_dev_inst* sdi) {
    struct dev_context* devc;
    gchar* trig_mode;
    ...
    case PROTOCOL_V1:
        uni_t_mso2_set_wait_event(devc, WAIT_TRIGGER);
        break;
    case PROTOCOL_V2:
        if (devc->data_source == DATA_SOURCE_LIVE) {
            if (uni_t_mso2_config_set(sdi, ":WAV:POIN:MODE NORMAL") != SR_OK)
                return SR_ERR;
            uni_t_mso2_set_wait_event(devc, WAIT_TRIGGER);
        }
        else {
            if (uni_t_mso2_config_set(sdi, ":STOP") != SR_OK)
                return SR_ERR;
            if (uni_t_mso2_config_set(sdi, ":WAV:POIN:MODE RAW") != SR_OK)
                return SR_ERR;
            if (sr_scpi_get_string(sdi->conn, ":TRIG:MODE?", &trig_mode) != SR_OK)
                return SR_ERR;
            ret = uni_t_mso2_config_set(sdi, ":TRIG:%s:SWE SING", trig_mode);
            g_free(trig_mode);
            if (ret != SR_OK)
                return SR_ERR;
            if (uni_t_mso2_config_set(sdi, ":RUN") != SR_OK)
                return SR_ERR;
            uni_t_mso2_set_wait_event(devc, WAIT_STOP);
        }
        break;
    ...
    case PROTOCOL_V4:
    case PROTOCOL_V5:
        if (first_frame && uni_t_mso2_config_set(sdi, ":WAV:FORM BYTE") != SR_OK)
            return SR_ERR;
        if (devc->data_source == DATA_SOURCE_LIVE) {
            if (first_frame && uni_t_mso2_config_set(sdi, ":WAV:MODE NORM") != SR_OK)
                return SR_ERR;
            ...
            if (devc->model->series->protocol == PROTOCOL_V3) {
                if (first_frame && uni_t_mso2_config_set(sdi, ":WAV:MODE RAW") != SR_OK)
                    return SR_ERR;
            }
            ...
            if (devc->data_source == DATA_SOURCE_LIVE && uni_t_mso2_config_set(sdi, ":SINGL") != SR_OK)
                return SR_ERR;
            uni_t_mso2_set_wait_event(devc, WAIT_STOP);
            if (devc->data_source == DATA_SOURCE_SEGMENTED &&
                devc->model->series->protocol <= PROTOCOL_V4)
                if (uni_t_mso2_config_set(sdi, "FUNC:WREP:FCUR %d", devc->num_frames + 1) != SR_OK)
                    return SR_ERR;
        }
        break;
    }
...
SR_PRIV int uni_t_mso2_channel_start(const struct sr_dev_inst* sdi) {
    struct dev_context* devc;
    struct sr_channel* ch;
    ...
    case PROTOCOL_V3:
        ...
        if (devc->data_source != DATA_SOURCE_LIVE) {
            if (uni_t_mso2_config_set(sdi, ":WAV:RES") != SR_OK)
                return SR_ERR;
            if (uni_t_mso2_config_set(sdi, ":WAV:BEG") != SR_OK)
                return SR_ERR;
        }
        break;
    case PROTOCOL_V4:
    case PROTOCOL_V5:
        if (ch->type == SR_CHANNEL_ANALOG) {
            ...
            if (uni_t_mso2_config_set(sdi, ":WAV:SOUR D%d",
                                      ch->index) != SR_OK)
                return SR_ERR;
        }

        if (first_frame && uni_t_mso2_config_set(sdi,
    ...
    uni_t_mso2_set_wait_event(devc, WAIT_BLOCK);
...
static int uni_t_mso2_read_header(struct sr_dev_inst* sdi) {
...
SR_PRIV int uni_t_mso2_receive(int fd, int revents, void* cb_data) {
    ...
    case WAIT_NONE:
        break;
    case WAIT_TRIGGER:
        if (uni_t_mso2_trigger_wait(sdi) != SR_OK)
            return TRUE;
        if (uni_t_mso2_channel_start(sdi) != SR_OK)
            return TRUE;
        return TRUE;
    case WAIT_BLOCK:
        if (uni_t_mso2_block_wait(sdi) != SR_OK)
            return TRUE;
        break;
    case WAIT_STOP:
        if (uni_t_mso2_stop_wait(sdi) != SR_OK)
            return TRUE;
        if (uni_t_mso2_check_stop(sdi) != SR_OK)
            return TRUE;
        if (uni_t_mso2_channel_start(sdi) != SR_OK)
            return TRUE;
        return TRUE;
    ...
    expected_data_bytes = ch->type == SR_CHANNEL_ANALOG ? devc->analog_frame_size : devc->digital_frame_size;

    if (devc->num_block_bytes == 0) {
        if (devc->model->series->protocol >= PROTOCOL_V4) {
            if (first_frame && uni_t_mso2_config_set(sdi, ":WAV:START %d",
                                                     devc->num_channel_bytes + 1) != SR_OK)
                return TRUE;
            if (first_frame && uni_t_mso2_config_set(sdi, ":WAV:STOP %d",
        ...
        if (devc->format == FORMAT_IEEE488_2) {
            ...
            len = uni_t_mso2_read_header(sdi);
            if (len == 0)
    ...
    if (devc->model->series->protocol == PROTOCOL_V3) {
        ...
        if (devc->data_source != DATA_SOURCE_LIVE)
             ...
             * firmware...
             */
            uni_t_mso2_config_set(sdi, ":WAV:END");
    }

    if (devc->channel_entry->next) {
        /* We got the frame for this channel, now get the next channel. */
        devc->channel_entry = devc->channel_entry->next;
        uni_t_mso2_channel_start(sdi);
    }
    else {
        ...
        if (devc->data_source == DATA_SOURCE_SEGMENTED &&
            ...
            int frames = 0;
            if (uni_t_mso2_config_set(sdi, "REC:CURR %d", devc->num_frames + 1) != SR_OK)
                return SR_ERR;
...
SR_PRIV int uni_t_mso2_get_dev_cfg(const struct sr_dev_inst* sdi) {
    struct dev_context* devc;
    struct sr_channel* ch;
    char* cmd;
    unsigned int i;
...
SR_PRIV int uni_t_mso2_get_dev_cfg_vertical(const struct sr_dev_inst* sdi) {
    struct dev_context* devc;
    char* cmd;
    unsigned int i;
    int res;
